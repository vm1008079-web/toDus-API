...
class ToDusFileMixin:
    ...
    def reserve_upload_url(self, token: str, size: int, file_type: FileType, file_name: str = "") -> tuple[str, str]:
        ...
        except (OSError, ConnectionLostError) as e:
            raise UploadError(f"Error de red al reservar URL: {e}") from e
        except Exception as e:
            raise UploadError(f"Error inesperado al reservar URL: {e}") from e
    ...
    def get_real_download_url(self, token: str, url: str) -> str:
        ...
        try:
            ...
            raise UploadError(f"Error de red al obtener URL real: {e}") from e
        except Exception as e:
            raise UploadError(f"Error inesperado al obtener URL real: {e}") from e
    ...
    def upload_file(self, token: str, data: bytes, file_type: FileType = FileType.FILE, progress_callback: Callable[[int, int], None] = None, file_name: str = "") -> str:
        try:
            ...
            raise UploadError(f"Error HTTP al subir archivo: {e}") from e
        except Exception as e:
            raise UploadError(f"Error inesperado al subir archivo: {e}") from e
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise UploadError(f"Descarga falló después de {max_retries} intentos: {e}") from e
                wait = 2 ** attempt
                logger.warning(f"Reintentando descarga ({attempt+1}/{max_retries}) en {wait}s: {e}")
                time.sleep(wait)
            except Exception as e:
                raise UploadError(f"Error inesperado en descarga: {e}") from e
    ...
    def download_file_to_folder(self, token: str, url: str, folder: str, filename: str = "") -> tuple[int, str]:
        ...
        try:
            ...
            real_url = self.get_real_download_url(token, url)
            if not real_url:
                raise UploadError(f"No se pudo obtener URL de descarga: {e}") from e
        ...
        try:
            ...
            raise UploadError(f"Error HTTP en descarga: {e}") from e
        except OSError as e:
            raise UploadError(f"Error de E/S en descarga: {e}") from e
