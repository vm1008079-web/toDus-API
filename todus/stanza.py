...
def delete_message(to: str, message_id: str, msg_id: str = "", msg_type: str = "c") -> str:
    """Eliminar mensaje."""
    mid = msg_id or _generate_msg_id()
    return (
        f"<m to='{to}' t='{msg_type}' i='{mid}' xmlns='jc'>"
        f"<deleted xmlns='deleted:n' i='{message_id}'/>"
        f"</m>"
    )


# ============================================================
# MENSAJES DE GRUPO (t='gc')
# ============================================================
...
def group_file_message(to: str, url: str, file_name: str, file_size: int, 
    ...
    """Archivo para grupo MUC Light. ORDEN: <file> → <k> → <b/>"""
    mid = msg_id or _generate_msg_id()
    fid = _generate_msg_id()
    name_esc = util.escape_xml(file_name)
    url_esc = util.escape_xml(url)
...
def group_image_message(to: str, url: str, file_name: str, file_size: int,
                        ...
                        caption: str = "", msg_id: str = "") -> str:
    """Imagen para grupo MUC Light. ATRIBUTOS: w, he (no width/height)"""
    mid = msg_id or _generate_msg_id()
    fid = _generate_msg_id()
    name_esc = util.escape_xml(file_name)
    url_esc = util.escape_xml(url)
...
def group_delete_message(to: str, message_id: str, msg_id: str = "") -> str:
